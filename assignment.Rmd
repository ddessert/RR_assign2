...
df$EVTYPE[grep("FREEZING RAIN/SNOW", df$EVTYPE)] <- "SLEET"
...
x <- ddply(df, .(EVTYPE, YEAR), summarize, 
           ...
           fatal.mean=mean(FATALITIES), 
           fatal.sd=sd(FATALITIES), 
           injury.sum=sum(INJURIES), 
           injury.mean=mean(INJURIES), 
           injury.sd=sd(INJURIES), 
           prop_dmg.sum=sum(PROPDMG),
           prop_dmg.mean=mean(PROPDMG),
           prop_dmg.sd=sd(PROPDMG)
           )
```
Top-N Ordered by property damage per event
```{r}
N <- 7      # Number of EVTYPE's to plot
top_propdmg <- x[order(x$prop_dmg.mean, decreasing=TRUE)[1:N],]
df_top_propdmg <- df[df$EVTYPE %in% top_propdmg$EVTYPE, ]
qplot(top_propdmg$fatal.mean, top_propdmg$injury.mean, size=top_propdmg$count, colour=top_propdmg$EVTYPE)
```

Top-N Ordered by fatalities per event
```{r}
head(x[order(x$fatal.mean, decreasing=TRUE),], N)
```

Top-N Ordered by injuries per event
```{r}
head(x[order(x$injury.mean, decreasing=TRUE),], N)
```
