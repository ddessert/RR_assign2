...
Count of events
```{r}
x <- ddply(df, .(EVTYPE), summarize, count=length(EVTYPE), fatal=sum(FATALITIES), injury=sum(INJURIES), prop_dmg=sum(PROPDMG))
```
Top-20 Ordered by property damage per event
```{r}
head(x[order(x$prop_dmg/x$count, decreasing=TRUE),], 20)
```

Top-20 Ordered by fatalities per event
```{r}
head(x[order(x$fatal/x$count, decreasing=TRUE),], 20)
```

Top-20 Ordered by injuries per event
```{r}
head(x[order(x$injury/x$count, decreasing=TRUE),], 20)
```
